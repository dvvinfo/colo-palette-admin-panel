  ...
  if (wager > 0) {
    ...
    for (i = 0; i < bet.length; i++) {
      if (bet[i].numbers == n && bet[i].type == t) {
        ...
        let chipColour =
          bet[i].amt < 5 ? 'red' : bet[i].amt < 10 ? 'blue' : bet[i].amt < 100 ? 'orange' : 'gold'
...
window.addEventListener('message', function (event) {
  if (event.data && event.data.type === 'SPIN_ROULETTE') {
    // Установить ставку (если нужно)
    if (typeof event.data.bet === 'number') {
      wager = event.data.bet
      currentBet = wager
    }
    // Запустить вращение
...
// Переопределяем win для отправки результата родителю
const origWin = win
win = function (winningSpin, winValue, betTotal) {
  origWin.apply(this, arguments)
  // Определяем результат
  let result = winValue > 0 ? 'win' : 'lose'
