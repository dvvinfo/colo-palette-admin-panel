...
window.addEventListener('message', function (event) {
  if (event.data && event.data.type === 'PLAY_WHEEL') {
    // Установить ставку (если нужно)
    window._wheelBet = typeof event.data.bet === 'number' ? event.data.bet : 0
    // Запустить спин (или новую игру)
    if (typeof init === 'function') {
      init()
    }
    // Автоматически крутим колесо и делаем попытку угадать букву (демо)
    setTimeout(() => {
      if (typeof spinAmount === 'function') {
        alert(spinAmount())
      }
      // Можно добавить автоматический guess, если нужно
    }, 500)
  }
})

// Переопределяем функцию, которая сообщает о победе/проигрыше, для отправки результата родителю
function sendWheelResult(result, winAmount) {
...
// Вызовите sendWheelResult('win', window._wheelBet) при победе, sendWheelResult('lose', 0) при проигрыше
