...
window.addEventListener('message', function (event) {
  if (event.data && event.data.type === 'PLAY_BLACKJACK') {
    // Сбросить игру
    BJdeal()
    // Установить ставку (если нужно)
    window._blackjackBet = typeof event.data.bet === 'number' ? event.data.bet : 0
    // Автоматически сыграть раунд: hit, stand, deal
    // Можно усложнить, если нужно, но для демо делаем простую автоигру
    setTimeout(() => {
      BJhit()
      setTimeout(() => {
        BJstand()
...
// Переопределяем showresults для отправки результата родителю
const origShowResults = showresults
showresults = function (winner) {
  origShowResults.apply(this, arguments)
  let result = 'draw'
  if (winner === You) result = 'win'
  else if (winner === Dealer) result = 'lose'
  let winAmount = 0
  if (result === 'win') winAmount = window._blackjackBet || 0
